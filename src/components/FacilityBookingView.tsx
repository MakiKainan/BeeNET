...
export default function FacilityBookingView({
  ...
  const toggleAvailability = (avail: string) => {
    if (selectedAvailabilities.includes(avail)) {
      setSelectedAvailabilities(selectedAvailabilities.filter(a => a !== avail));
    } else {
  ...
  const filteredFacilities = facilities.filter((fac) => {
    ...
    const matchesSport = selectedSports.length === 0 || selectedSports.some((sport) => {
      ...
      if (sport === 'Tennis' && fac.title.toLowerCase().includes('aquatic')) return false; // avoid false triggers
      return false;
    });
  ...
  return (
    <div className="space-y-8 animate-fade-in select-none text-left">
      ...
      {/* Main filter/search layout section */}
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        
        {/* Left Sidebar Filters panels */}
        <aside className="w-full lg:w-72 shrink-0">
          <div className="glass-card rounded-2xl p-6 space-y-6">
            
            <div className="flex items-center justify-betweenpb-2 border-b border-[#c1c6d7]/20 flex justify-between items-center">
              <h3 className="text-xs font-bold uppercase tracking-widest text-[#717786]">Filters</h3>
              <button 
                onClick={resetFilters}
                className="text-[#0059bb] text-xs font-bold hover:underline cursor-pointer"
              >
                Clear all
              </button>
            </div>
                  ...
                  return (
                    <label key={sport} className="flex items-center gap-3 cursor-pointer group">
                      <input 
                        ...
                        className="rounded border-[#717786]/50 text-[#fe6b00] focus:ring-[#fe6b00]/15 h-5 w-5 cursor-pointer accent-[#fe6b00]"
                      />
            ...
            {/* Price maximum per hour metrics */}
            ...
            {/* Availability badges selector list */}
            <div className="space-y-2.5">
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                {['Instant Booking', 'Evening Slots', 'Weekend Only'].map((avail) => {
                  const isActive = selectedAvailabilities.includes(avail);
                  return (
                    <button
                      key={avail}
                      type="button"
                ...
                >
                  ...
                  <div className="h-52 relative overflow-hidden bg-[#ebeef3]">
                    ...
                    <div className="absolute bottom-3.5 right-3.5 bg-white/95 backdrop-blur-md px-3 py-1.5 rounded-xl text-xs font-black text-[#a04100] shadow-sm tracking-tight border border-white/40">
                      Rp {fac.pricePerHour.toLocaleString('id-ID')} / hr
                    </div>
                  </div>

                  {/* Booking details info package */}
                  <div className="p-5 flex flex-col flex-grow text-left justify-between space-y-4">
                    ...
                    {/* CTA operations button */}
                    <div className="flex gap-2 pt-2">
                      {bBtnDisabled ? (
                        <button 
                          disabled
                          className="w-full bg-[#d7dadf] text-[#717786] font-extrabold py-3 rounded-xl text-xs text-center cursor-not-allowed uppercase tracking-wider"
                        >
                          Unavailable
                        </button>
                      ) : (
                        <>
                          ...
                          <button 
                            onClick={() => alert(`Added ${fac.title} to your favorited venues library!`)}
